...
## Alternative Channel Codes (for backward compatibility)

The following codes are alternative names used in some data files:

fendianqi_V: 车辆分线器端的电压(V) [Alias for Vehicle_Harness_Splitter_V]
fendianqi_A: 车辆分线器端的电流(A) [Alias for Vehicle_Harness_Splitter_A]
yasuoji_A: 压缩机输入端电流(A) [Alias for ACCM_A]
...
│   ├── statistics.xlsx
│   └── 87_超车80-140_FM_V_8.39VPP_0.61kHz-0.106V.png  # Channel in filename should match folder
├── LV_A/                    # Sensor channel = LV_A (from folder name)
│   ├── statistics.xlsx
│   └── 87_超车80-140_LV_A_12.5VPP_1.2kHz-2.1A.png     # Channel in filename should match folder
```
